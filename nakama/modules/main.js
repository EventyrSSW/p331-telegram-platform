...
function rpcGetLeaderboard(ctx, logger, nk, payload) {
  ...
  try {
    ...
    // First check if user is in the already-fetched records
    for (var m = 0; m < records.length; m++) {
      if (records[m].odredacted === userId) {
        myRecord = records[m];
        myRank = records[m].rank;
        break;
      }
    }
    ...
    try {
      ...
      if (statsResult && statsResult.objects) {
        for (var j = 0; j < statsResult.objects.length; j++) {
          totalGamesPlayed += Number(statsResult.objects[j].value.gamesPlayed) || 0;
        }
      }
